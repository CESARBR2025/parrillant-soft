...
export function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  return (
    <aside
      className={`hidden lg:flex flex-col bg-card border-r border-border/60 transition-all duration-200 ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      {/* Logo */}
      <div className={`flex items-center h-16 px-4 border-b border-border/60 ${collapsed ? 'justify-center' : 'gap-3'}`}>
        <Image
        ...
        <button
          onClick={onToggle}
          className={`p-1.5 rounded-lg text-muted hover:text-body hover:bg-bg-base transition-colors ${collapsed ? 'ml-0' : 'ml-auto'}`}
        >
